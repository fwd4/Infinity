...
def parse_args():
    ...
    parser.add_argument('--prompt_folder', type=str, default='evaluation/dpg_bench/prompts', help='Folder containing txt files with prompts')
...
# Add this new function to read prompts from txt files
def load_prompts_from_folder(prompt_folder):
    """Load prompts from txt files in the specified folder"""
    prompts = []
    for txt_file in glob.glob(os.path.join(prompt_folder, '*.txt')):
        with open(txt_file, 'r') as f:
            prompt = f.readline().strip()  # Read single line from each file
            if prompt:  # Only add non-empty prompts
                prompts.append({'prompt': prompt, 'id': os.path.splitext(os.path.basename(txt_file))[0]})
    return prompts
...
if __name__ == '__main__':
    ...
    # Load prompts from folder instead of metadata file
    metadatas = load_prompts_from_folder(args.prompt_folder)
    
    # In distributed environment, each process handles a portion of data
    if is_distributed:
    ...
    for index, metadata in enumerate(tqdm.tqdm(metadatas, desc=f"Rank {rank} processing prompts")):
        ...
        if 'infinity' in args.model_type:
            # Images are already tensors for infinity model
            image_tensors = images  # No conversion needed
            # Convert BGR to RGB and ensure correct tensor format (B, C, H, W)
            image_tensors = [img.permute(2, 0, 1).flip(0).float() if len(img.shape) == 3 else img.float() for img in image_tensors]
            torchvision.utils.save_image(
                image_tensors,
                os.path.join(args.outdir, f"{metadata['id']}.jpg"),
                nrow=args.n_samples//2,
                padding=0,
                normalize=True  # Don't normalize since we're handling it ourselves
            )
        else:
            ...
            torchvision.utils.save_image(
                image_tensors,
                os.path.join(sample_path, f"{metadata['id']}.jpg"),
                nrow=args.n_samples//2,
                padding=0,
                normalize=True
            )
    
        # # Remove the individual image saving loop since we only want the grid
