...
def verify_processed_parquet(processed_captions, expected_len, shard_id):
    if len(processed_captions) != expected_len:
        print(f"\nWarning: Sample count mismatch in shard-{shard_id:05d}!")
        print(
            f"Input samples: {expected_len}, Processed samples: {len(processed_captions)}"
        )
        return False
    ...
    if empty_captions:
        print(
            f"\nWarning: Found {len(empty_captions)} empty captions in shard-{shard_id:05d}"
        )
        return True
    return True
...
def gather_samples_to_process(input_dataset, processed_dataset=None):
    ...
    if processed_dataset is not None:
        ...
        for idx, (image, caption, enhanced) in enumerate(zip(
        ...
        for idx, (image, caption) in enumerate(zip(input_dataset["image"], input_dataset["caption"])):
...
def update_processed_dataset(processed_dataset, input_dataset, processed_samples, sample_type):
    """Update processed dataset with new results."""
    if sample_type == "empty":
        # Update existing dataset's enhanced captions
        ...
        # Create new dataset for missing or new samples
        new_samples = {k: [] for k in input_dataset.features}
        new_samples["enhanced_caption"] = []
        
        for idx, caption in processed_samples.items():
            for key in input_dataset.features:
                if key != "enhanced_caption":
                    new_samples[key].append(input_dataset[key][idx])
            new_samples["enhanced_caption"].append(caption)
            
        if any(new_samples.values()):
...
def process_parquet(parquet_path, template, output_dir):
    t0 = time.time()
    shard_id = int(osp.basename(parquet_path).split("-")[1].split(".")[0])
    input_path, failed_path, output_path = get_paths(template, shard_id)

    # Load datasets
    input_dataset = load_dataset("parquet", data_files={"train": input_path}, split="train")
    processed_dataset = None if not os.path.exists(output_path) else load_dataset(
        "parquet", data_files={"train": output_path}, split="train"
    )

    # Gather all samples that need processing
    ...
    nsfw_filtered = 0
    error_count = 0
    sample_type = samples_to_process[0]["type"]  # All samples will be of same type

    for sample in tqdm(samples_to_process, desc=f"Processing shard-{shard_id:05d}"):
        enhanced_caption, error_code = process_image_and_caption(sample["image"], sample["caption"])
    ...
    if successful_samples:
        processed_dataset = update_processed_dataset(
            processed_dataset, input_dataset, successful_samples, sample_type
        )
        os.makedirs(osp.dirname(output_path), exist_ok=True)
        processed_dataset.to_parquet(output_path)

    # Print statistics and verify
    print(f"Processed {len(successful_samples)} samples, filtered {nsfw_filtered} NSFW images, {error_count} errors")
    tt = time.time()
    print(f"Processed parquet-{shard_id:05d}, total time {tt-t0}s")
    verify_processed_parquet(
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='Process dataset with specified templates')
    parser.add_argument('--templates', type=str, default="1.0",
                       help='Comma-separated template values (e.g., "1.0,0.8,0.6")')
    parser.add_argument('--workers', type=int, default=8,
                       help='Number of worker processes in the pool')
    ...
    for i in range(0, n_parquets, parquet_round):
        ...
        with multiprocessing.Pool(processes=args.workers) as pool:
            results = pool.starmap(
                process_parquet, [(p[1], p[0], output_dir) for p in parquets[start:end]]
            )
        time.sleep(20)
...
def process_image_and_caption(image, caption):
    ...
    try:
        ...
        result = caption if response["done_reason"] == "stop" else response["message"]["content"]
