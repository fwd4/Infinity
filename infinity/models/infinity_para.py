...
class Infinity(nn.Module):
    ...
    def autoregressive_infer_cfg(
        ...
        for si, pn in enumerate(scale_schedule):   # si: i-th segment
            if si <= 5:
                ...
                if si != num_stages_minus_1:
                    ...
                    if si == 5:
                        summed_codes_5 = summed_codes.clone()  # Save summed_codes for stage 8
                        continue
                ...
                last_stage = F.interpolate(summed_codes, size=vae_scale_schedule[si], mode=vae.quantizer.z_interplote_up) # [B, d, 1, h, w] or [B, d, 1, 2h, 2w]
                ######################### 2.1 #############################
                last_stage = last_stage.squeeze(-3) # [B, d, h, w] or [B, d, 2h, 2w]
                if self.apply_spatial_patchify: # patchify operation
                    last_stage = torch.nn.functional.pixel_unshuffle(last_stage, 2) # [B, 4d, h, w]
                ...
                if si != num_stages_minus_1:
                    ...
                    summed_codes += test_partial_code
                else:
                    summed_codes += codes
        ...
        with open(f'outputs/codes/test_para5_combined_data_{category}.pkl', 'wb') as f:
