...
case $task in
    ...
    gen_eval)
        rewrite_prompt=2
        out_dir="${out_dir_root}/gen_eval_${sub_fix}_rewrite_prompt${rewrite_prompt}_flex_attn${use_flex_attn}_round2_real_rewrite_prefix${prefix}_si_para${si_para}-ratio_list${ratio_list}-kv_opt${kv_opt}_orign"
        test_gen_eval
        break
        ;;
    mjhq30k_fid)
