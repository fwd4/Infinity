...
def compute_contribution(A, B, C, D, E, R, base, eps=1e-8):  
    ...
    dE_flat = dE.reshape(32, -1) # [32, 4096]  
    E_flat = E.reshape(32, -1)  # [32, 4096]  


    # 计算E的范数 (32维特征向量的长度)  
    dE_norm = torch.norm(dE_flat, dim=0) + eps  # [4096]  
    dE_norm_squared = dE_norm ** 2              # [4096] 
    ...
    for v in v_list:  
        ...
        # 计算向量投影: <v_j, dE> / ||dE||  
        dot_product = torch.sum(v_flat * dE_flat, dim=0)  # [4096]  
        
        # 标量投影 s_j (原始贡献率)  
        scalar_proj = dot_product / dE_norm  # [4096]  
        contributions.append(scalar_proj)  

        # 归一化贡献率 r_j (各分量和为1)  
        ratio = dot_product / dE_norm_squared  # [4096]  
    ...
    # 重新整形为空间维度 [64, 64]  
    contributions_map = contributions.reshape(4, 64, 64)  
    norm_contrib_map = norm_contribs.reshape(4, 64, 64)  
...
def visualize_contributions(results, save_path=None, example_pixel=(32, 32), batch_idx=None, fig=None):  
    ...
    # 提取结果  
    raw_contrib = results['raw_contribution']       # [4, 64, 64]  
    norm_contrib = results['normalized_contribution'] # [4, 64, 64]  
    dominant_map = results['dominant_map']          # [64, 64]  
    names = results['component_names']              # ['A', 'B', 'C', 'D']  
    ...
    component_colors = ['#FF5733', '#33FF57', '#3357FF', '#F033FF']  # 分量颜色  
...
def batch_analyze_tensors(tensor_path, output_folder, num_batches=20, example_pixel=(32, 32)):  
    ...
    # 确保张量形状正确  
    if orin_tensor.shape[1:] != (5, 32, 64, 64):  
    ...
    pdf_filename = f'{output_folder}/{name_without_extension}_dE_comb_all_batches.pdf'  # 创建完整的 PDF 文件名  
    
    # 创建一个PDF文件用于保存所有批次的图像  
    # pdf_filename = f'{output_folder}/{os.path.basename(tensor_path).split('.')[0]}_comb_all_batches.pdf'  
    
    # 限制处理批次数量  
    num_batches = min(num_batches, orin_tensor.shape[0])  
    ...
    with PdfPages(pdf_filename) as pdf:  
        ...
        for batch_idx in range(num_batches):  
            ...
            # 提取当前批次的张量 [5, 32, 64, 64]  
            batch_tensor = orin_tensor[batch_idx]  
            
            # 分割张量，得到A、B、C、D和E  
            A, B, C, D, E = batch_tensor.split(1, dim=0)  # [1, 32, 64, 64]  

            R = A + B + C + D
            # 计算基础矩阵  
            base = E - (A + B + C + D)  
            
            # 删除第一维，将[1, 32, 64, 64]转换为[32, 64, 64]  
            A = A.squeeze(0)  
            B = B.squeeze(0)  
            C = C.squeeze(0)  
            D = D.squeeze(0)  
